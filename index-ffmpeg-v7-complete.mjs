...
console.log("🎵 Discord Music Bot - FFmpeg v7 Compatible + AI Chat");
...
client.on("messageCreate", async (message) => {
    ...
    if (command === "help") {
        const embed = new EmbedBuilder()
            .setColor('#0099FF')
            .setTitle('🎵 Kugy Music Bot - FFmpeg v7 + AI Chat')
            ...
            .setFooter({ text: 'Kugy Bot v1.1.0 - FFmpeg v7 + AI Ready!' });

        message.reply({ embeds: [embed] });
    }

    // AI Chat dengan OpenRouter - MENGGUNAKAN MODEL YANG COST-EFFECTIVE
    if (message.mentions.has(client.user.id) || message.content.startsWith("!chat ")) {
        ...
        if (!prompt) {
            return message.reply("❌ Silakan berikan pesan untuk chat dengan Kugy AI!\nContoh: `!chat Halo Kugy!`");
        }

        try {
            console.log(`🤖 AI Chat request from ${message.author.username}: ${prompt}`);
            ...
            const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                ...
                body: JSON.stringify({
                    // MENGGUNAKAN MODEL YANG COST-EFFECTIVE DAN BAGUS
                    model: "meta-llama/llama-3.1-8b-instruct:free", // Model gratis yang bagus
                    messages: [
                        {
                            role: "system",
                            content: "You are Kugy AI, a cute, supportive, and humble Indonesian assistant. Always reply warmly and motivatively in Indonesian. You also help with music bot commands and FFmpeg v7 troubleshooting. Keep responses concise and friendly."
                        },
            ...
            const fallbackResponses = [
                ...
                "Hi! 🤖 Kugy AI lagi maintenance sebentar. Untuk music commands, coba `!help`!",
                "Hai! ✨ Kugy AI sedang istirahat. Kalau mau putar musik, pakai `!play <lagu>` ya!",
                "Hello! 🎵 Kugy AI lagi offline. Tapi music bot masih jalan kok! Coba `!play`!"
            ];
...
app.get('/api/status', (req, res) => {
    res.json({
        ...
        ai: {
            ...
            provider: "OpenRouter",
            status: process.env.OPENROUTER_API_KEY ? 'Available' : 'Not configured'
        }
    });
});
...
client.login(process.env.DISCORD_TOKEN)
    .then(() => {
        ...
        console.log(`🤖 AI Chat: ${process.env.OPENROUTER_API_KEY ? 'Ready with Llama 3.1 8B' : 'Not configured'}`);
    })
    .catch(error => {
...
client.on("ready", () => {
    ...
    console.log(`🤖 AI Chat ready with Llama 3.1 8B Instruct (Free)`);
